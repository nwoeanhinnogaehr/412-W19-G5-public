            ...
            if rospy.get_time() > self.stop_time:
                ...
                if self.red_visible <= 0 and red_moments['m00'] > self.red_cutoff:
                    ...
                    if white_moments['m00'] > 0:
                        ...
                        twist = Twist()
                        twist.linear.x = turtlebot_speed
                        twist.angular.z = -float(output) / 50
                        cmd_vel_pub.publish(twist)
                    else:
                        print("NO LINE")
...
def do_box_push():
    ...
    (goal, box) = result
    print(goal, box)
    if goal > box: # if goal is closer to window
        turn_turtlebot(math.pi/2)
        drive_for_dist(0.3,0,0.7)
        align_to_box()
        drive_for_dist(-0.3,0,0.1)
        turn_turtlebot(math.pi/2)
        drive_for_dist(-0.3,0,0.35)
        align_to_box()
...
def align_to_red_line():
    ...
    if left_moments['m00'] > 3 and right_moments['m00'] > 3:
